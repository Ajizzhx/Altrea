...
export const ornaments = {
  ...
  archFrame(opts = {}) {
    ...
    return `<svg viewBox="0 0 ${width} ${height}" fill="none" xmlns="http://www.w3.org/2000/svg" 
      ...
      <!-- Inner frame -->
      <path d="M34,${height-34} L34,106 Q34,34 ${width/2},34 Q${width-34},34 ${width-34},106 L${width-34},${height-34} Z"
        stroke="${color}" stroke-width="1.2" stroke-opacity="${opacity * 0.75}" fill="none"/>
      <!-- Corner accents TL -->
      <path d="M20,${height-20} L20,${height-50} M20,${height-20} L50,${height-20}" stroke="${color}" stroke-width="2.5" stroke-opacity="${opacity}"/>
      <!-- Corner accents TR -->
      <path d="M${width-20},${height-20} L${width-20},${height-50} M${width-20},${height-20} L${width-50},${height-20}" stroke="${color}" stroke-width="2.5" stroke-opacity="${opacity}"/>
      <!-- Bottom center ornament -->
      <polygon points="${width/2},${height-6} ${width/2-7},${height-20} ${width/2+7},${height-20}" 
        fill="${color}" fill-opacity="${opacity}"/>
      <!-- Top peak -->
      <circle cx="${width/2}" cy="18" r="5" fill="none" stroke="${color}" stroke-width="2" stroke-opacity="${opacity}"/>
      <circle cx="${width/2}" cy="18" r="2" fill="${color}" fill-opacity="${opacity}"/>
      ${anim}
    </svg>`;
  },
